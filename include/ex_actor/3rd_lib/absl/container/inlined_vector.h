...
//
// An `ex_actor::embedded_3rd::absl::InlinedVector<T, N>` specifies the default capacity `N` as one of
// its template parameters. Instances where `size() <= N` hold contained
// elements in inline space. Typically `N` is very small so that sequences that
// are expected to be short do not require allocations.
//
// An `ex_actor::embedded_3rd::absl::InlinedVector` does not usually require a specific allocator. If
// the inlined vector grows beyond its initial constraints, it will need to
// allocate (as any normal `std::vector` would). This is usually performed with
// the default allocator (defined as `std::allocator<T>`). Optionally, a custom
// allocator type may be specified as `A` in `ex_actor::embedded_3rd::absl::InlinedVector<T, N, A>`.

#ifndef EX_ACTOR_ABSL_CONTAINER_INLINED_VECTOR_H_
#define EX_ACTOR_ABSL_CONTAINER_INLINED_VECTOR_H_
...
#include "ex_actor/3rd_lib/absl/algorithm/algorithm.h"
#include "ex_actor/3rd_lib/absl/base/attributes.h"
#include "ex_actor/3rd_lib/absl/base/internal/iterator_traits.h"
...
#include "ex_actor/3rd_lib/absl/container/internal/inlined_vector.h"
#include "ex_actor/3rd_lib/absl/hash/internal/weakly_mixed_integer.h"
#include "ex_actor/3rd_lib/absl/memory/memory.h"
...
// -----------------------------------------------------------------------------
//
// An `ex_actor::embedded_3rd::absl::InlinedVector` is designed to be a drop-in replacement for
// `std::vector` for use cases where the vector's size is sufficiently small
// that it can be inlined. If the inlined vector does grow beyond its estimated
// capacity, it will trigger an initial allocation on the heap, and will behave
// as a `std::vector`. The API of the `ex_actor::embedded_3rd::absl::InlinedVector` within this file is
// designed to cover the same API footprint as covered by `std::vector`.
template <typename T, size_t N, typename A = std::allocator<T>>
class EX_ACTOR_ABSL_ATTRIBUTE_WARN_UNUSED InlinedVector {
  static_assert(N > 0, "`ex_actor::embedded_3rd::absl::InlinedVector` requires an inlined capacity.");
 ...
 public:
  ...
  InlinedVector(const InlinedVector& other, const allocator_type& allocator)
    ...
    if (ex_actor::embedded_3rd::absl::is_trivially_copy_constructible<value_type>::value &&
  ...
  InlinedVector(InlinedVector&& other) noexcept(
      ex_actor::embedded_3rd::absl::allocator_is_nothrow<allocator_type>::value ||
    ...
    if (ex_actor::embedded_3rd::absl::is_trivially_relocatable<value_type>::value &&
        std::is_same<A, std::allocator<value_type>>::value) {
      storage_.MemcpyFrom(other.storage_);
  ...
  InlinedVector(
      ...
      const allocator_type&
          allocator) noexcept(ex_actor::embedded_3rd::absl::allocator_is_nothrow<allocator_type>::value)
      : storage_(allocator) {
    // Fast path: if the value type can be trivially relocated (i.e. moved from
    // and destroyed), and we know the allocator doesn't do anything fancy, then
    // it's safe for us to simply adopt the contents of the storage for `other`
  ...
  pointer data() noexcept EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
  ...
  //
  // NOTE: only elements within [`data()`, `data() + size()`) are valid.
  const_pointer data() const noexcept EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    return storage_.GetIsAllocated() ? storage_.GetAllocatedData()
                                     : storage_.GetInlinedData();
  }
  ...
  reference operator[](size_type i) EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
  ...
  const_reference operator[](size_type i) const EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    EX_ACTOR_ABSL_HARDENING_ASSERT(i < size());
    return data()[i];
  }
  ...
  reference at(size_type i) EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    if (EX_ACTOR_ABSL_PREDICT_FALSE(i >= size())) {
  ...
  // NOTE: if `i` is not within the required range of `InlinedVector::at(...)`,
  // in both debug and non-debug builds, `std::out_of_range` will be thrown.
  const_reference at(size_type i) const EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    if (EX_ACTOR_ABSL_PREDICT_FALSE(i >= size())) {
      base_internal::ThrowStdOutOfRange(
  ...
  reference front() EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    EX_ACTOR_ABSL_HARDENING_ASSERT(!empty());
    return data()[0];
  }

  // Overload of `InlinedVector::front()` that returns a `const_reference` to
  // the first element of the inlined vector.
  const_reference front() const EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    EX_ACTOR_ABSL_HARDENING_ASSERT(!empty());
    return data()[0];
  }
  ...
  reference back() EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    EX_ACTOR_ABSL_HARDENING_ASSERT(!empty());
    return data()[size() - 1];
  }
  ...
  const_reference back() const EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    EX_ACTOR_ABSL_HARDENING_ASSERT(!empty());
    return data()[size() - 1];
  }
  ...
  iterator begin() noexcept EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND { return data(); }

  // Overload of `InlinedVector::begin()` that returns a `const_iterator` to
  // the beginning of the inlined vector.
  const_iterator begin() const noexcept EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    return data();
  }
  ...
  iterator end() noexcept EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    return data() + size();
  }

  // Overload of `InlinedVector::end()` that returns a `const_iterator` to the
  // end of the inlined vector.
  const_iterator end() const noexcept EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    return data() + size();
  }
  ...
  const_iterator cbegin() const noexcept EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    return begin();
  }
  ...
  const_iterator cend() const noexcept EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    return end();
  }
  ...
  reverse_iterator rbegin() noexcept EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    return reverse_iterator(end());
  }

  // Overload of `InlinedVector::rbegin()` that returns a
  // `const_reverse_iterator` from the end of the inlined vector.
  const_reverse_iterator rbegin() const noexcept EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
  ...
  reverse_iterator rend() noexcept EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    return reverse_iterator(begin());
  }

  // Overload of `InlinedVector::rend()` that returns a `const_reverse_iterator`
  // from the beginning of the inlined vector.
  const_reverse_iterator rend() const noexcept EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
  ...
  // vector.
  const_reverse_iterator crend() const noexcept EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    return rend();
  }
  ...
  InlinedVector& operator=(const InlinedVector& other) {
    if (EX_ACTOR_ABSL_PREDICT_TRUE(this != std::addressof(other))) {
  ...
  void resize(size_type n) {
    EX_ACTOR_ABSL_HARDENING_ASSERT(n <= max_size());
  ...
  iterator insert(const_iterator pos,
                  const_reference v) EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
  ...
  iterator insert(const_iterator pos,
                  value_type&& v) EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
  ...
  iterator insert(const_iterator pos, size_type n,
                  const_reference v) EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    EX_ACTOR_ABSL_HARDENING_ASSERT(pos >= begin());
    EX_ACTOR_ABSL_HARDENING_ASSERT(pos <= end());

    if (EX_ACTOR_ABSL_PREDICT_TRUE(n != 0)) {
...
#pragma GCC diagnostic pop
  ...
  iterator insert(const_iterator pos, ForwardIterator first,
                  ForwardIterator last) EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    EX_ACTOR_ABSL_HARDENING_ASSERT(pos >= begin());
    EX_ACTOR_ABSL_HARDENING_ASSERT(pos <= end());

    if (EX_ACTOR_ABSL_PREDICT_TRUE(first != last)) {
  ...
  iterator insert(const_iterator pos, InputIterator first,
                  InputIterator last) EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    EX_ACTOR_ABSL_HARDENING_ASSERT(pos >= begin());
    EX_ACTOR_ABSL_HARDENING_ASSERT(pos <= end());
  ...
  iterator emplace(const_iterator pos,
                   Args&&... args) EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    EX_ACTOR_ABSL_HARDENING_ASSERT(pos >= begin());
    EX_ACTOR_ABSL_HARDENING_ASSERT(pos <= end());
...
#pragma GCC diagnostic pop
  ...
  template <typename... Args>
  reference emplace_back(Args&&... args) EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
  ...
  iterator erase(const_iterator pos) EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    EX_ACTOR_ABSL_HARDENING_ASSERT(pos >= begin());
    EX_ACTOR_ABSL_HARDENING_ASSERT(pos < end());
...
#pragma GCC diagnostic pop
  ...
  iterator erase(const_iterator from,
                 const_iterator to) EX_ACTOR_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    EX_ACTOR_ABSL_HARDENING_ASSERT(from >= begin());
    EX_ACTOR_ABSL_HARDENING_ASSERT(from <= to);
    EX_ACTOR_ABSL_HARDENING_ASSERT(to <= end());

    if (EX_ACTOR_ABSL_PREDICT_TRUE(from != to)) {
 ...
 private:
  ...
  friend H AbslHashValue(H h, const ex_actor::embedded_3rd::absl::InlinedVector<TheT, TheN, TheA>& a);
  ...
  void MoveAssignment(MemcpyPolicy, InlinedVector&& other) {
    ...
    static_assert(ex_actor::embedded_3rd::absl::is_trivially_destructible<value_type>::value, "");
  ...
  void DestroyExistingAndAdopt(InlinedVector&& other) {
    EX_ACTOR_ABSL_HARDENING_ASSERT(other.storage_.GetIsAllocated());
...
void swap(ex_actor::embedded_3rd::absl::InlinedVector<T, N, A>& a,
          ex_actor::embedded_3rd::absl::InlinedVector<T, N, A>& b) noexcept(noexcept(a.swap(b))) {
  a.swap(b);
}
...
template <typename T, size_t N, typename A>
bool operator==(const ex_actor::embedded_3rd::absl::InlinedVector<T, N, A>& a,
                const ex_actor::embedded_3rd::absl::InlinedVector<T, N, A>& b) {
  auto a_data = a.data();
  auto b_data = b.data();
...
// `AbslHashValue(...)`
//
// Provides `ex_actor::embedded_3rd::absl::Hash` support for `ex_actor::embedded_3rd::absl::InlinedVector`. It is uncommon to
// call this directly.
template <typename H, typename T, size_t N, typename A>
H AbslHashValue(H h, const ex_actor::embedded_3rd::absl::InlinedVector<T, N, A>& a) {
...
#endif  // EX_ACTOR_ABSL_CONTAINER_INLINED_VECTOR_H_
