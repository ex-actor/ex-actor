...
#ifndef EX_ACTOR_ABSL_CONTAINER_INTERNAL_INLINED_VECTOR_H_
#define EX_ACTOR_ABSL_CONTAINER_INTERNAL_INLINED_VECTOR_H_
...
#include "ex_actor/3rd_lib/absl/container/internal/compressed_tuple.h"
#include "ex_actor/3rd_lib/absl/memory/memory.h"
#include "ex_actor/3rd_lib/absl/meta/type_traits.h"
#include "ex_actor/3rd_lib/absl/types/span.h"
...
template <typename A>
using IsSwapOk = ex_actor::embedded_3rd::absl::type_traits_internal::IsSwappable<ValueType<A>>;

template <typename A,
          bool IsTriviallyDestructible =
              ex_actor::embedded_3rd::absl::is_trivially_destructible<ValueType<A>>::value &&
...
template <typename A,
          bool IsOverAligned =
              (alignof(ValueType<A>) > EX_ACTOR_ABSL_INTERNAL_DEFAULT_NEW_ALIGNMENT)>
...
void ConstructElements(ex_actor::embedded_3rd::absl::type_identity_t<A>& allocator,
                       Pointer<A> construct_first, ValueAdapter& values,
                       SizeType<A> construct_size) {
  for (SizeType<A> i = 0; i < construct_size; ++i) {
    EX_ACTOR_ABSL_INTERNAL_TRY { values.ConstructNext(allocator, construct_first + i); }
    EX_ACTOR_ABSL_INTERNAL_CATCH_ANY {
      DestroyAdapter<A>::DestroyElements(allocator, construct_first, i);
      EX_ACTOR_ABSL_INTERNAL_RETHROW;
    }
  }
}
...
class Storage {
 public:
  ...
  using MoveAssignmentPolicy = ex_actor::embedded_3rd::absl::conditional_t<
      ...
      ex_actor::embedded_3rd::absl::conjunction<ex_actor::embedded_3rd::absl::is_trivially_move_assignable<ValueType<A>>,
                        ex_actor::embedded_3rd::absl::is_trivially_destructible<ValueType<A>>,
                        std::is_same<A, std::allocator<ValueType<A>>>>::value,
      MemcpyPolicy,
      ...
      // not.
      ex_actor::embedded_3rd::absl::conditional_t<IsMoveAssignOk<A>::value, ElementwiseAssignPolicy,
                          ElementwiseConstructPolicy>>;

  // The policy to be used specifically when swapping inlined elements.
  using SwapInlinedElementsPolicy = ex_actor::embedded_3rd::absl::conditional_t<
      ...
      ex_actor::embedded_3rd::absl::conjunction<ex_actor::embedded_3rd::absl::is_trivially_relocatable<ValueType<A>>,
                        std::is_same<A, std::allocator<ValueType<A>>>>::value,
      MemcpyPolicy,
      ex_actor::embedded_3rd::absl::conditional_t<IsSwapOk<A>::value, ElementwiseSwapPolicy,
                          ElementwiseConstructPolicy>>;
  ...
  ~Storage() {
    ...
    // maybe not even that).
    if (ex_actor::embedded_3rd::absl::is_trivially_destructible<ValueType<A>>::value &&
        std::is_same<A, std::allocator<ValueType<A>>>::value) {
...
#if EX_ACTOR_ABSL_INTERNAL_HAVE_MIN_GNUC_VERSION(12, 0)
#pragma GCC diagnostic push
...
#pragma GCC diagnostic pop
  ...
  // EX_ACTOR_ABSL_ATTRIBUTE_NO_SANITIZE_CFI is used because the memory pointed to may be
  ...
  // as a bad cast.
  EX_ACTOR_ABSL_ATTRIBUTE_NO_SANITIZE_CFI Pointer<A> GetInlinedData() {
    return reinterpret_cast<Pointer<A>>(data_.inlined.inlined_data);
  }

  EX_ACTOR_ABSL_ATTRIBUTE_NO_SANITIZE_CFI ConstPointer<A> GetInlinedData() const {
  ...
  EX_ACTOR_ABSL_ATTRIBUTE_NOINLINE void InitFrom(const Storage& other);
  ...
  void SubtractSize(SizeType<A> count) {
    EX_ACTOR_ABSL_HARDENING_ASSERT(count <= GetSize());
    ...
    //
    {
      ...
      EX_ACTOR_ABSL_HARDENING_ASSERT(
          ...
          (std::is_same<A, std::allocator<V>>::value &&
           (
               // First case above
               ex_actor::embedded_3rd::absl::is_trivially_relocatable<V>::value ||
               // Second case above
               (ex_actor::embedded_3rd::absl::is_trivially_move_assignable<V>::value &&
                ex_actor::embedded_3rd::absl::is_trivially_destructible<V>::value) ||
               // Third case above
               (ex_actor::embedded_3rd::absl::is_trivially_copy_constructible<V>::value ||
                ex_actor::embedded_3rd::absl::is_trivially_copy_assignable<V>::value))));
    }
 ...
 private:
  EX_ACTOR_ABSL_ATTRIBUTE_NOINLINE void DestroyContents();
  ...
  template <typename... Args>
  EX_ACTOR_ABSL_ATTRIBUTE_NOINLINE Reference<A> EmplaceBackSlow(Args&&... args);
...
void Storage<T, N, A>::InitFrom(const Storage& other) {
  ...
  EX_ACTOR_ABSL_HARDENING_ASSERT(n > 0);  // Empty sources handled handled in caller.
  ConstPointer<A> src;
  Pointer<A> dst;
  ...
  if (ex_actor::embedded_3rd::absl::is_trivially_copy_constructible<ValueType<A>>::value &&
      std::is_same<A, std::allocator<ValueType<A>>>::value) {
...
auto Storage<T, N, A>::Initialize(ValueAdapter values,
  ...
  EX_ACTOR_ABSL_HARDENING_ASSERT(!GetIsAllocated());
  EX_ACTOR_ABSL_HARDENING_ASSERT(GetSize() == 0);
...
auto Storage<T, N, A>::Assign(ValueAdapter values,
  ...
  ex_actor::embedded_3rd::absl::Span<ValueType<A>> assign_loop;
  ex_actor::embedded_3rd::absl::Span<ValueType<A>> construct_loop;
  ex_actor::embedded_3rd::absl::Span<ValueType<A>> destroy_loop;
...
auto Storage<T, N, A>::Insert(ConstIterator<A> pos, ValueAdapter values,
  ...
  if (new_size > storage_view.capacity) {
    ...
    ex_actor::embedded_3rd::absl::Span<ValueType<A>> move_construction = {
        storage_view.data + move_construction_destination_index,
        new_size - move_construction_destination_index};

    Pointer<A> move_assignment_values = storage_view.data + insert_index;
    ex_actor::embedded_3rd::absl::Span<ValueType<A>> move_assignment = {
        storage_view.data + insert_end_index,
        move_construction_destination_index - insert_end_index};

    ex_actor::embedded_3rd::absl::Span<ValueType<A>> insert_assignment = {move_assignment_values,
                                                  move_construction.size()};

    ex_actor::embedded_3rd::absl::Span<ValueType<A>> insert_construction = {
...
auto Storage<T, N, A>::EmplaceBack(Args&&... args) -> Reference<A> {
  ...
  if (EX_ACTOR_ABSL_PREDICT_TRUE(n != storage_view.capacity)) {
...
auto Storage<T, N, A>::Erase(ConstIterator<A> from,
  ...
  if (ex_actor::embedded_3rd::absl::is_trivially_relocatable<ValueType<A>>::value &&
...
auto Storage<T, N, A>::Reserve(SizeType<A> requested_capacity) -> void {
  ...
  if (EX_ACTOR_ABSL_PREDICT_FALSE(requested_capacity <= storage_view.capacity)) return;
...
auto Storage<T, N, A>::ShrinkToFit() -> void {
  ...
  if (EX_ACTOR_ABSL_PREDICT_FALSE(storage_view.size == storage_view.capacity)) return;
...
auto Storage<T, N, A>::Swap(Storage* other_storage_ptr) -> void {
  using std::swap;
  EX_ACTOR_ABSL_HARDENING_ASSERT(this != other_storage_ptr);
...
#endif  // EX_ACTOR_ABSL_CONTAINER_INTERNAL_INLINED_VECTOR_H_
