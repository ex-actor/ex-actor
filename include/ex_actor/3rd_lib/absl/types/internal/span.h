...
//
#ifndef EX_ACTOR_ABSL_TYPES_INTERNAL_SPAN_H_
#define EX_ACTOR_ABSL_TYPES_INTERNAL_SPAN_H_
...
template <typename C>
using HasSize =
    std::is_integral<ex_actor::embedded_3rd::absl::decay_t<decltype(std::declval<C&>().size())>>;
...
using HasData =
    std::is_convertible<ex_actor::embedded_3rd::absl::decay_t<decltype(GetData(std::declval<C&>()))>*,
                        T* const*>;

// Extracts value type from a Container
template <typename C>
struct ElementType {
  using type = typename ex_actor::embedded_3rd::absl::remove_reference_t<C>::value_type;
};
...
EX_ACTOR_ABSL_INTERNAL_CONSTEXPR_SINCE_CXX20 bool EqualImpl(SpanT<T> a, SpanT<T> b) {
  static_assert(std::is_const<T>::value, "");
  return std::equal(a.begin(), a.end(), b.begin(), b.end());
}

template <template <typename> class SpanT, typename T>
EX_ACTOR_ABSL_INTERNAL_CONSTEXPR_SINCE_CXX20 bool LessThanImpl(SpanT<T> a, SpanT<T> b) {
...
template <typename T>
struct IsView<
    T, ex_actor::embedded_3rd::absl::void_t<decltype(span_internal::GetData(std::declval<const T&>()))>,
    ex_actor::embedded_3rd::absl::void_t<decltype(span_internal::GetData(std::declval<T&>()))>> {
 private:
...
#endif  // EX_ACTOR_ABSL_TYPES_INTERNAL_SPAN_H_
