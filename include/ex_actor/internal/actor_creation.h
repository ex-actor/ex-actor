...
class ActorRef {
 public:
  ...
  ActorRef(uint32_t node_index, uint64_t actor_id, internal::TypeErasedActor* actor)
 ...
 private:
  ...
  internal::TypeErasedActor* type_erased_actor_ = nullptr;
};
...
class ActorRegistry {
 public:
  ...
  ~ActorRegistry() {
    ...
    // bulk destroy actors
    auto destroy_msg = std::make_unique<internal::DestroyMessage>();
  ...
  ActorRef<UserClass> CreateActor(ActorConfig config, Args&&... args) {
    ...
    auto actor = std::make_unique<internal::Actor<UserClass, Scheduler>>(scheduler_, std::move(config),
                                                                         std::forward<Args>(args)...);
 ...
 private:
  ...
  std::unordered_map<uint64_t, std::unique_ptr<internal::TypeErasedActor>> actor_id_to_actor_;
};
}  // namespace ex_actor
