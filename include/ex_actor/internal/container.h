...
// contention on Push entirely. Pop uses a bitmap + LightweightSemaphore (spin-then-futex)
// for blocking — no mutex involved. The bitmap allows O(1) lookup of the highest-priority
// non-empty bucket via std::countr_zero, avoiding a linear scan of all buckets.
template <class T>
class BucketedPriorityQueue {
 public:
  explicit BucketedPriorityQueue(uint32_t max_priorities) : max_priorities_(max_priorities), buckets_(max_priorities) {
    EXA_THROW_CHECK(max_priorities <= 64)
        << fmt_lib::format("max_priorities={} exceeds bitmap capacity of 64", max_priorities);
  }
  ...
  void Push(T value, uint32_t priority) {
    ...
    if ((non_empty_bitmap_.load(std::memory_order::relaxed) & bit) == 0) {
      non_empty_bitmap_.fetch_or(bit, std::memory_order::release);
    }
    sema_.signal();
  }

  // Non-blocking dequeue: tries to grab the highest-priority item without waiting.
  // Returns nullopt if no item is immediately available. Consumes a semaphore permit
  // on success to keep the count in sync.
  std::optional<T> TryPop() {
    T value;
    uint64_t bitmap = non_empty_bitmap_.load(std::memory_order::acquire);
    while (bitmap != 0) {
      uint32_t candidate = static_cast<uint32_t>(std::countr_zero(bitmap));
      if (buckets_[candidate].try_dequeue(value)) {
        sema_.tryWait();
        return value;
      }
      non_empty_bitmap_.fetch_and(~(uint64_t{1} << candidate), std::memory_order::relaxed);
      bitmap &= ~(uint64_t{1} << candidate);
    }
    return std::nullopt;
  }
  ...
  std::optional<T> Pop(uint64_t timeout_ms) {
    ...
    T value;
    // Fast path: use bitmap to jump directly to the highest-priority non-empty bucket.
    uint64_t bitmap = non_empty_bitmap_.load(std::memory_order::acquire);
    while (bitmap != 0) {
      uint32_t candidate = static_cast<uint32_t>(std::countr_zero(bitmap));
      if (buckets_[candidate].try_dequeue(value)) {
      ...
      // Bucket appeared empty — speculatively clear the bit. A concurrent Push will
      // re-set it via fetch_or if needed.
      non_empty_bitmap_.fetch_and(~(uint64_t{1} << candidate), std::memory_order::relaxed);
      bitmap &= ~(uint64_t{1} << candidate);
    }
    // Fallback: bitmap was empty but semaphore fired (race with a concurrent Push that
    // enqueued but hasn't set the bitmap bit yet, or spurious try_dequeue failures).
 ...
 private:
  ...
  std::atomic<uint64_t> non_empty_bitmap_ {0};
};
