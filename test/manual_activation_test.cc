...
class ManualActivationActor {
 public:
  ...
  bool ExActorOnUnsafeMessageSlotFilled(size_t /*unsafe_message_slot_index*/) {
    ...
    if (cnt == 10) {
      ex_actor::internal::log::Info("activate the actor");
      return true;
    }
    return false;
  }
...
TEST(ManualActivationTest, ManualActivation) {
  ...
  auto main_coro = []() -> exec::task<void> {
    ...
    ex_actor::internal::log::Info("Sending to unsafe slot 0 to 8, should not activate the actor");
    for (int i = 0; i < 9; i++) {
      async_scope.spawn(manual_activation_actor.UnsafeMessageSlot(i).Send<&ManualActivationActor::Ping>());
    }
    std::this_thread::sleep_for(std::chrono::seconds(1));
    ex_actor::internal::log::Info("Checking count, should be 0");
    uint32_t count = co_await counter.Send<&Counter::GetCount>();
    EXPECT_EQ(count, 0);

    ex_actor::internal::log::Info("Sending to unsafe slot 9, now has 10 messages, should activate the actor");
